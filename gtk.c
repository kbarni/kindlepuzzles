...
{
    message_box(parent, thegame.wintitle, msg, false, MB_OK);
}
...
static void resize_fe(frontend *fe)
{
    ...
    gtk_drawing_area_size(GTK_DRAWING_AREA(fe->area), x, y);
    {
        ...
        if(thegame.needs_keyboard) {
            printf("Setting window size (kb) to %d / %d", screen_width, (screen_height - screen_height/KBFACTOR));
            gtk_window_resize(GTK_WINDOW(fe->window), screen_width, (screen_height - screen_height/KBFACTOR));
        } else {
            printf("Setting window size (nonkb) to %d / %d",  req.width, req.height);
...
static void menu_save_event(GtkMenuItem *menuitem, gpointer data)
{
	...
	if ((fp = fopen(name, "r")) != NULL) {
	    ...
	    sprintf(buf, "Are you sure you want to overwrite the"
		    " file \"%.*s\"?",
		    FILENAME_MAX, name);
	    if (!message_box(fe->window, thegame.wintitle, buf, true, MB_YESNO))
...
# define ABOUT_PARAMS                                               \
    ...
    sprintf(textbuf,
	    "%.200s\n\n"
	    "from Simon Tatham's Portable Puzzle Collection\nKindle port by kbarni\nwww.github.com/kbarni/puzzles\n"
	    "%.500s\n\n%s", thegame.name, ver,thegame.helptext);

    message_box(fe->window, thegame.wintitle, textbuf, true, MB_OK);
#endif
}
    ...
    char *arg, int argtype, char **error, bool headless)
{
    ...
    gtk_window_set_title(GTK_WINDOW(fe->window), thegame.wintitle);
...
*/
	...
	*((int*)errbuf)=0;
    ...
    if(thegame.needs_keyboard)
        g_signal_connect_after(G_OBJECT(fe->window), "focus_in_event",
