...
const struct game thegame = {
    "Galaxies","L:A_N:application_PC:T_ID:com.kbarni.galaxies", "Draw lines along grid edges so as to divide the grid up into connected regions of squares.\n\
    Every region should have two-way rotational symmetry, should contain exactly one dot which is in its centre, and should contain no lines separating two \
    of its own squares from each other. A region satisfying all of these requirements will be automatically highlighted.\Tap on a grid edge to add or\
    remove a line.",
