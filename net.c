...
const struct game thegame = {
    "Net","L:A_N:application_PC:T_ID:com.kbarni.net", "Rotate the grid squares so that they all join up into a single connected network with no loops.\n\
    Tap on a square to rotate it anticlockwise. Squares connected to the middle square are lit up. Aim to light up every square in the grid (not just the \
    endpoint blobs).\nWhen this gets too easy, select a 'wrapping' variant from the Type menu to enable grid lines to run off one edge of the playing area \
