...
struct game const thegame = {
    "Range","L:A_N:application_PC:T_ID:com.kbarni.range", " Colour some squares black, so as to meet the following conditions:\n\
    1.No two black squares are orthogonally adjacent.\n2. No group of white squares is separated from the rest of the grid by black squares.\n\
    3. Each numbered cell can see precisely that many white squares in total by looking in all four orthogonal directions, counting itself. \
    (Black squares block the view. So, for example, a 2 clue must be adjacent to three black squares or grid edges, and in the fourth direction \
    there must be one white square and then a black one beyond it.)\n\
    Tap on a square to cycle between black, dot (if you know it should not be black) and gray. ",
