...
const struct game thegame = {
    "Netslide","L:A_N:application_PC:T_ID:com.kbarni.netslide", "Slide the grid squares around so that they all join up into a single connected \
    network with no loops.\nClick on the arrows at the edges of the grid to move a row or column left, right, up or down. The square that falls off \
    the end of the row comes back on the other end.\nSquares connected to the middle square are lit up. Aim to light up every square in the grid (not \
    just the endpoint blobs).\nConnecting across a barrier line is forbidden. On harder levels, there are fewer barriers, which makes it harder rather than easier! ",
