...
const struct game thegame = {
    "Dominosa","L:A_N:application_PC:T_ID:com.kbarni.dominosa", "Tile the rectangle with dominoes (1x2 rectangles) so that every possible \
        domino appears exactly once (that is, every possible pair of numbers, including doubles).\nClick between two adjacent numbers to \
        place or remove a domino. Right-click to place a line between numbers if you think a domino definitely cannot go there. Dominoes \
        light up red if two identical ones appear on the grid. ",
    false,
    default_params,
